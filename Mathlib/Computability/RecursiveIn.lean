...
* `Nat.RecursiveIn O f`: An inductive definition representing that a partial function `f : ℕ →. ℕ`
  is partial recursive given access to a set of oracles `O`.
* `RecursiveIn O f`: A partial function `f : α →. σ` between `Primcodable` types is recursive in
  `O`.
* `ComputableIn O f`: A total function `f : α → σ` is computable in `O`.
...
theorem partrec_of_oracle {f : ℕ →. ℕ} {O}
  ...
  induction hf with repeat {constructor}
  | oracle g hg => exact hO g hg
...
/-- A partial function `f : α →. σ` is recursive in `O` if the encoded form is
`Nat.RecursiveIn O`. -/
...
/-- If a partial function is partial recursive, then it is `Nat.RecursiveIn` any oracle set. -/
...
lemma of_eq (hf : RecursiveIn O f') {g : α →. σ}
...
theorem subst {O O' : Set (ℕ →. ℕ)} {f₀ : α →. σ} (hf : RecursiveIn O f₀)
...
@[gcongr]
theorem mono {O₁ O₂ : Set (ℕ →. ℕ)} {f₀ : α →. σ} (hsub : O₁ ⊆ O₂)
...
theorem partrec_of_oracle {f₀ : α →. σ} {O : Set (ℕ →. ℕ)}
...
lemma partrec_of_const {f₀ : α →. σ} {s} (hf : RecursiveIn {fun _ => s} f₀) : Partrec f₀ :=
...
@[simp]
lemma recursiveIn_empty_iff {f₀ : α →. σ} :
...
theorem partrec_iff_forall_recursiveIn_singleton {f₀ : α →. σ} :
